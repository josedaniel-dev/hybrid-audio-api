...
# ────────────────────────────────────────────────
# Bucket listing v2 (paginated, normalized paths)
# ────────────────────────────────────────────────
def list_bucket_contents_v2(
    prefix: str = "",
    page_size: int = 1000,
    max_pages: int = 10,
) -> List[str]:
    """
    List bucket contents with optional prefix and basic pagination.

    Args:
        prefix: Optional prefix to filter blobs (e.g. "stems/name/").
        page_size: Approximate number of blobs per page.
        max_pages: Maximum number of pages to iterate.

    Returns:
        List of blob names (strings) without any bucket information.
    """
    if not is_gcs_enabled():
        _safe_print("⚠️ GCS disabled or credentials missing.")
        return []
    ...
    try:
        ...
        blobs_iter = bucket.list_blobs(prefix=safe_prefix, page_size=page_size)
        results: List[str] = []

        for page_index, page in enumerate(blobs_iter.pages):
            for blob in page:
                # Blob name is already relative to bucket; we just ensure it is a clean string.
        ...
        _safe_print(f"⚠️ Failed to list bucket contents (v2): {e}")
        return []
