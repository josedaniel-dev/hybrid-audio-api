...
# ────────────────────────────────────────────────
# v5.3 — GCS Mode Matrix (LOCAL / SAFE / PROD)
#     Required for:
#       • /cache/consistency_report
#       • /cache/verify_and_repair
#       • rotational_engine sync logic
# ────────────────────────────────────────────────

# Allowed values:
#   LOCAL → no GCS usage, always offline
#   SAFE  → optional GCS; do not fail if unavailable
#   PROD  → GCS required; failures bubble to API 503
GCS_MODE = os.getenv("GCS_MODE", "LOCAL").upper().strip()
if GCS_MODE not in ("LOCAL", "SAFE", "PROD"):
    GCS_MODE = "LOCAL"


def get_gcs_mode() -> str:
    """
    Returns the effective GCS mode:
        LOCAL → GCS always disabled
        SAFE  → try to use GCS if possible
        PROD  → GCS must be available
    """
    return GCS_MODE
...
def is_gcs_enabled() -> bool:
    """
    Determines whether GCS is effectively active based on the mode.

    LOCAL:
        Always False (GCS fully disabled)

    SAFE:
        Enabled only if bucket + credentials exist

    PROD:
        Same as SAFE, but callers must enforce failure if False
    """
    if GCS_MODE == "LOCAL":
        return False

    try:
        creds_ok = bool(GOOGLE_APPLICATION_CREDENTIALS) and Path(GOOGLE_APPLICATION_CREDENTIALS).exists()
        bucket_ok = bool(GCS_BUCKET)
        return bucket_ok and creds_ok
    except Exception:
        return False
